...
export async function uploadAdminFileBuffer({
  organization,
  category,
  name,
  buffer,
  ...
  if (buffer.length > MAX_ADMIN_FILE_BYTES) {
    throw new Error("產生的檔案超過 25 MB");
  }
  await ensureAdminFileBucket();
  const safeName = cleanFileName(name);
  const path = `${organization}/${category}/${randomUUID()}--${encodedName(safeName)}`;
...
export async function removeAdminFilePath(path) {
  if (!path) return;
  const { error } = await supabaseAdmin.storage
    .from(ADMIN_FILE_BUCKET)
    .remove([path]);
  if (error) throw new Error("移除產生的檔案失敗");
}
...
export async function saveAdminFileOrder(organization, category, orderedPaths) {
  ...
  const positions = new Map(nextPaths.map((path, index) => [path, index]));
  return files.sort(
    (left, right) =>
      (positions.get(left.path) ?? Number.MAX_SAFE_INTEGER) -
      (positions.get(right.path) ?? Number.MAX_SAFE_INTEGER)
...
export async function deleteAdminFile({ organization, category, path }) {
  const folder = `${organization}/${category}`;
  const files = await listAdminFiles(organization, category);
  if (!files.some((file) => file.path === path)) {
    throw new Error("找不到要刪除的檔案");
  }
  if (!String(path).startsWith(`${folder}/`) || path.endsWith(`/${ADMIN_FILE_ORDER_NAME}`)) {
    throw new Error("檔案路徑不正確");
  }
  const { error } = await supabaseAdmin.storage
    .from(ADMIN_FILE_BUCKET)
    .remove([path]);
  if (error) throw new Error("刪除檔案失敗");
  return saveAdminFileOrder(
    organization,
    category,
    files.filter((file) => file.path !== path).map((file) => file.path)
  );
}
