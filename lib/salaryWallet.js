...
function isMonthlyCustomerServiceBonus(config, bonus) {
  if (config.appKey !== "qiunai") return false;
  const title = String(bonus?.title || "");
  return (
    title === "客服接待件數薪資" ||
    title.includes("客服底薪")
  );
...
function buildCustomerServicePeriod(year, month) {
  const endDay = getLastDayOfMonth(year, month);
  const nextMonth = new Date(Date.UTC(year, month + 1, 1));
  const releaseYear = nextMonth.getUTCFullYear();
  const releaseMonth = nextMonth.getUTCMonth();

  return {
    key: `${monthText(year, month)}-customer-service`,
    label: `${monthText(year, month)} 客服薪資`,
    startIso: taipeiToUtcIso(year, month, 1, 0, 0, 0),
    endIso: taipeiToUtcIso(year, month, endDay, 23, 59, 59),
    releaseIso: taipeiToUtcIso(releaseYear, releaseMonth, 2, 0, 0, 0),
    settlementDate: dateText(releaseYear, releaseMonth, 2),
  };
}

function getDueCustomerServicePeriods(now = new Date(), monthsBack = 8) {
  const parts = getTaipeiParts(now);
  const periods = [];
  const startTime = new Date(`${getWalletStartDateText()}T00:00:00+08:00`).getTime();

  for (let offset = monthsBack; offset >= 0; offset -= 1) {
    const source = new Date(Date.UTC(parts.year, parts.month - offset, 1));
    const period = buildCustomerServicePeriod(
...
export function calculateWithdrawFees(amount, monthlyWithdrawalCount = 0) {
  ...
  return {
    serviceFee,
    payoutAmount:
      Math.round((requestedAmount - serviceFee) * 100) / 100,
  };
}
...
async function loadAllPages(loadPage, pageSize = 1000) {
  const rows = [];
...
async function markSettled(
  ...
  for (let index = 0; index < uniqueIds.length; index += 500) {
    ...
    if (markOrderStatus) {
      settledValues.status = "已入帳";
      settledValues.paid_at = period.releaseIso;
    }

    const { error } = await supabaseAdmin
      .from(table)
      .update(settledValues)
...
export async function settleSalaryWallet(supabaseAdmin, config) {
  ...
  for (const period of periods) {
    ...
    for (const bonus of bonuses || []) {
      if (isMonthlyCustomerServiceBonus(config, bonus)) continue;
      const discordId = String(bonus.discord_id || "").trim();
      const amount = numberValue(bonus.amount);
      if (!discordId || amount === 0) continue;
  ...
  for (const period of getDueCustomerServicePeriods()) {
    const { data: bonuses, error: bonusError } = await supabaseAdmin
      .from(config.bonusTable)
      .select(config.bonusSelect)
      .not("discord_id", "is", null)
    ...
    if (bonusError) {
      console.error("[salary wallet] load customer service salary failed", bonusError);
      throw new Error("讀取待入帳客服薪資失敗");
    }

    const customerServiceBonuses = (bonuses || []).filter((bonus) =>
      isMonthlyCustomerServiceBonus(config, bonus),
    );
    const entries = [];
    const bonusIds = [];

    for (const bonus of customerServiceBonuses) {
      const discordId = String(bonus.discord_id || "").trim();
      const amount = numberValue(bonus.amount);
      if (!discordId || amount === 0) continue;
...
export async function getSalaryWalletSummary(supabaseAdmin, appKey, discordId) {
  ...
  const pendingWithdrawn = withdrawRequests
    ...
    .reduce((sum, request) => sum + numberValue(request.amount), 0);
  const pendingRequestCount = withdrawRequests.filter(
  ...
  for (const entry of walletEntries) {
    const month = String(
      entry.settlement_date || entry.created_at || ""
    ).slice(0, 7);
    if (month) ensureMonth(month).deposited += numberValue(entry.amount);
  }

  for (const request of withdrawRequests) {
    const requestedAt = new Date(request.requested_at || 0);
    if (!Number.isFinite(requestedAt.getTime())) continue;
    const taipeiDate = new Date(requestedAt.getTime() + TAIPEI_OFFSET_MS);
    const month = `${taipeiDate.getUTCFullYear()}-${String(
      taipeiDate.getUTCMonth() + 1
    ).padStart(2, "0")}`;
    const summary = ensureMonth(month);
    if (request.status === "approved") {
      summary.withdrawn += numberValue(request.amount);
    } else if (request.status === "pending") {
      summary.pendingWithdrawn += numberValue(request.amount);
    }
  }
  ...
  return {
    ...
    entries: walletEntries.slice(0, 120),
    requests: withdrawRequests.slice(0, 500),
    pendingRequest:
