...
export default function AdminFilesPanel({ apiPath }: { apiPath: string }) {
  ...
  const loadFiles = useCallback(async () => { try { const token = await getToken(); const response = await fetch(`${apiPath}?category=${category}`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || "讀取檔案失敗"); setFiles(payload.files || []); setCanUpload(Boolean(payload.canUpload)); } catch (error) { console.error("load admin files failed", error); alert(error instanceof Error ? error.message : "讀取檔案失敗"); } finally { setLoading(false); } }, [apiPath, category, getToken]);
  ...
  async function moveFile(index: number, offset: -1 | 1) { const targetIndex = index + offset; if (targetIndex < 0 || targetIndex >= files.length || sorting) return; const previous = files; const next = [...files]; [next[index], next[targetIndex]] = [next[targetIndex], next[index]]; setFiles(next); setSorting(true); try { const token = await getToken(); const response = await fetch(apiPath, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ category, orderedPaths: next.map((file) => file.path) }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || "調整檔案排序失敗"); setFiles(payload.files || next); } catch (error) { setFiles(previous); alert(error instanceof Error ? error.message : "調整檔案排序失敗"); } finally { setSorting(false); } }
  return <main className="min-h-screen p-4 sm:p-7"><div className="mx-auto max-w-[1400px] space-y-5">
    <header className="rounded-3xl bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><span className="rounded-2xl bg-pink-100 p-3 text-pink-600"><FileArchive size={24}/></span><div><h1 className="text-2xl font-black text-[#3f2947]">資料下載</h1><p className="mt-1 text-sm text-[#80647d]">管理內部營運與員工相關文件。</p></div></div><div className="mt-6 grid gap-3 sm:grid-cols-2">{categories.map(({ key, label, icon: Icon }) => <button key={key} type="button" onClick={() => { setLoading(true); setCategory(key); }} className={`flex items-center gap-3 rounded-2xl border px-5 py-4 text-left font-black transition ${category === key ? "border-pink-300 bg-pink-500 text-white shadow-md shadow-pink-100" : "border-pink-100 bg-pink-50 text-[#73516f] hover:border-pink-200"}`}><Icon size={20}/>{label}</button>)}</div></header>
    {canUpload ? <section className="rounded-3xl border border-pink-100 bg-white p-5 shadow-sm"><h2 className="flex items-center gap-2 font-black text-[#3f2947]"><Upload size={19} className="text-pink-500"/>上傳到「{category === "operations" ? "營運相關" : "員工相關"}」</h2><div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]"><input key={fileInputKey} type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.ppt,.pptx,.txt,.rtf,.jpg,.jpeg,.png,.webp,.gif,.heic,.zip,.rar,.7z" onChange={(event) => setSelectedFile(event.target.files?.[0] || null)}/><button type="button" onClick={uploadFile} disabled={!selectedFile || uploading} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-pink-500 px-6 py-3 font-black text-white hover:bg-pink-600 disabled:opacity-40">{uploading ? <Loader2 size={18} className="animate-spin"/> : <Upload size={18}/>} {uploading ? "上傳中…" : "上傳檔案"}</button></div><p className="mt-3 text-xs text-[#80647d]">單一檔案上限 25 MB；僅指定帳號可以上傳。</p></section> : <p className="rounded-2xl border border-pink-100 bg-white px-5 py-4 text-sm font-semibold text-[#80647d]">此管理員帳號為查看與下載權限。</p>}
    <section className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="border-b border-pink-100 px-6 py-5"><h2 className="text-lg font-black text-[#3f2947]">{category === "operations" ? "營運相關" : "員工相關"}檔案</h2>{canUpload ? <p className="mt-1 text-xs font-semibold text-pink-500">使用上下箭頭調整顯示順序，排序會自動儲存。</p> : null}</div>{loading ? <p className="flex items-center justify-center gap-2 p-12 text-sm text-[#a36b9e]"><Loader2 size={18} className="animate-spin"/>讀取中…</p> : files.length ? <div className="divide-y divide-pink-50">{files.map((file, index) => <article key={file.path} className="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="shrink-0 rounded-xl bg-pink-50 p-2.5 text-pink-500"><FileText size={20}/></span><div className="min-w-0"><p className="truncate font-bold text-[#3f2947]">{file.name}</p><p className="mt-1 text-xs text-[#a36b9e]">{formatBytes(file.size)} · {formatDate(file.createdAt)}</p></div></div><div className="flex shrink-0 items-center gap-2">{canUpload ? <><button type="button" aria-label={`上移 ${file.name}`} onClick={() => moveFile(index, -1)} disabled={sorting || index === 0} className="rounded-xl border border-pink-100 p-2.5 text-pink-500 hover:border-pink-300 disabled:opacity-30"><ArrowUp size={16}/></button><button type="button" aria-label={`下移 ${file.name}`} onClick={() => moveFile(index, 1)} disabled={sorting || index === files.length - 1} className="rounded-xl border border-pink-100 p-2.5 text-pink-500 hover:border-pink-300 disabled:opacity-30"><ArrowDown size={16}/></button></> : null}<button type="button" onClick={() => downloadFile(file)} disabled={downloading === file.path} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#3f2947] px-4 py-2 text-sm font-black text-white hover:bg-pink-500 disabled:opacity-50">{downloading === file.path ? <Loader2 size={16} className="animate-spin"/> : <Download size={16}/>}下載</button></div></article>)}</div> : <p className="p-12 text-center text-sm text-[#a36b9e]">這個分類目前沒有檔案</p>}</section>
  </div></main>;
}
