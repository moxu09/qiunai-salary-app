  ...
  return <main className="min-h-screen p-4 sm:p-7"><div className="mx-auto max-w-[1400px] space-y-5">
    ...
    <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
      <div className="border-b border-pink-100 px-6 py-5">
        ...
        {canUpload ? <p className="mt-1 text-xs font-semibold text-pink-500">按住拖曳把手移動檔案，放開後會自動儲存順序。</p> : null}
      </div>
      {loading ? (
          ...
          {files.map((file) => (
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-2">
                <button type="button" onClick={() => downloadFile(file)} disabled={downloading === file.path} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#3f2947] px-4 py-2 text-sm font-black text-white hover:bg-pink-500 disabled:opacity-50">{downloading === file.path ? <Loader2 size={16} className="animate-spin"/> : <Download size={16}/>}下載</button>
              </div>
            </article>
          ))}
        </div>
