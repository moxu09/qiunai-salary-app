...
export default function AdminFilesPanel({ apiPath }: { apiPath: string }) {
  ...
  return <main className="min-h-screen p-4 sm:p-7"><div className="mx-auto max-w-[1400px] space-y-5">
    ...
    <section className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="border-b border-pink-100 px-6 py-5"><h2 className="text-lg font-black text-[#3f2947]">{category === "operations" ? "營運相關" : "員工相關"}檔案</h2></div>{loading ? <p className="flex items-center justify-center gap-2 p-12 text-sm text-[#a36b9e]"><Loader2 size={18} className="animate-spin"/>讀取中…</p> : files.length ? <div className="divide-y divide-pink-50">{files.map((file) => <article key={file.path} className="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="shrink-0 rounded-xl bg-pink-50 p-2.5 text-pink-500"><FileText size={20}/></span><div className="min-w-0"><p className="truncate font-bold text-[#3f2947]">{file.name}</p><p className="mt-1 text-xs text-[#a36b9e]">{formatBytes(file.size)} · {formatDate(file.createdAt)}</p></div></div><button type="button" onClick={() => downloadFile(file)} disabled={downloading === file.path} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#3f2947] px-4 py-2 text-sm font-black text-white hover:bg-pink-500 disabled:opacity-50">{downloading === file.path ? <Loader2 size={16} className="animate-spin"/> : <Download size={16}/>}下載</button></article>)}</div> : <p className="p-12 text-center text-sm text-[#a36b9e]">這個分類目前沒有檔案</p>}</section>
  </div></main>;
}
