...
export default function AdminSalaryPage() {
  ...
  async function deleteSalaryAdjustment(bonus: BonusItem) {
    ...
    const settledWarning = bonus.wallet_settled_at
      ...
      : "";
    const ok = window.confirm(
      `確定要刪除「${bonus.note || bonus.title || itemLabel}」這筆${itemLabel}嗎？${settledWarning}`
    );
    if (!ok) return;
    ...
    try {
      ...
      const response = await fetch("/api/qiunai/salary-adjustments", {
        method: "DELETE",
        headers: {
  ...
  async function approveWorkReport(report: WorkReport) {
    ...
    const regularRate = getStaffSalaryRate(staff, report.ended_at);
    const isTip =
  ...
  return (
    <main className="salary-overview-page min-h-screen bg-[#fff7fb] px-5 py-6 text-[#3f2947]">
      <div className="salary-overview-content mx-auto max-w-7xl space-y-5">
        ...
        <section className="salary-template space-y-5">
          <WorkReportReviewPanel
            appKey="qiunai"
            accent="pink"
            onApprove={approveWorkReport}
            reviewApiPath="/api/qiunai/work-reports/review"
          />
          ...
          <div className="salary-filter-panel rounded-[28px] border border-pink-100 bg-white p-5 shadow-sm shadow-pink-100">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-black text-[#3f2947]">
                時間範圍查詢
              </h2>
            </div>
          ...
          <div className="salary-personal-panel rounded-[28px] border border-pink-100 bg-white p-5 shadow-sm shadow-pink-100">
            <div className="flex items-center gap-2">
              <WalletCards className="text-pink-500" size={20} />
              <h2 className="text-lg font-black text-[#3f2947]">
                陪陪個人薪資明細
              </h2>
            </div>
                ...
                {selectedDetailStaff && selectedDetailSummary ? (
                  <div>
                    <div className="flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-pink-50 text-[#7b4f85]">
                        <UserRound size={22} />
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#92778f]">
                  ...
                  <div className="py-8 text-center text-sm text-[#80647d]">
                    請選擇一位陪陪
                  </div>
                )}
              </div>
            ...
            {selectedDetailStaff ? (
              <div className="mt-6 grid gap-6 xl:grid-cols-2">
                <div className="overflow-x-auto rounded-2xl border border-pink-100 bg-pink-50">
                  <div className="border-b border-pink-100 p-4">
                    <h3 className="font-bold">此陪陪訂單明細</h3>
                  </div>

                  {selectedDetailOrders.length === 0 ? (
                    <div className="p-8 text-center text-sm text-[#80647d]">
                      此時間範圍內沒有訂單
                    </div>
                  ) : (
                    <table className="min-w-[850px] w-full text-left text-sm">
                      <thead className="bg-white/10 text-[#6f526d]">
                        <tr>
                        ...
                        {selectedDetailBonuses.map((bonus) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <button
                                onClick={() =>
                                  void deleteSalaryAdjustment(bonus)
                                }
                                disabled={deletingBonusId === bonus.id}
                                className="inline-flex items-center gap-2 rounded-xl bg-red-500 px-3 py-2 text-xs font-semibold text-white hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-50"
                              >
          ...
          <div className="salary-commission-panel rounded-[28px] border border-pink-100 bg-white p-5 shadow-sm shadow-pink-100">
            <h2 className="text-lg font-black text-[#3f2947]">員工抽成檔位</h2>
          ...
          <div className="salary-order-form rounded-[28px] border border-pink-100 bg-white p-5 shadow-sm shadow-pink-100">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-black text-[#3f2947]">新增訂單</h2>
            </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <button
                onClick={addOrder}
                className="mt-6 flex items-center justify-center gap-2 rounded-xl bg-pink-500 px-4 py-3 font-semibold hover:bg-pink-400"
              >
                <Plus size={18} />
                新增訂單
              </button>
          ...
          <div className="salary-pay-panel rounded-[28px] border border-pink-100 bg-white p-5 shadow-sm shadow-pink-100">
            <h2 className="text-lg font-black text-[#3f2947]">發薪管理</h2>
          ...
          <div className="salary-orders-table overflow-x-auto rounded-[28px] border border-pink-100 bg-white shadow-sm shadow-pink-100">
            {loading ? (
              <div className="p-8 text-center text-[#80647d]">載入中...</div>
            ) : orders.length === 0 ? (
              <div className="p-8 text-center text-[#80647d]">
                此時間範圍內沒有薪資訂單
              </div>
            ) : (
          ...
          {editingOrder && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
              <div className="w-full max-w-lg rounded-[28px] border border-pink-100 bg-white p-6 text-[#3f2947] shadow-2xl">
                <h2 className="text-lg font-black text-[#3f2947]">
                  修改薪資訂單
                </h2>
                ...
                <div className="mt-6 flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 rounded-xl bg-slate-200 text-slate-700 px-4 py-3 font-bold hover:bg-slate-300"
                  >
                    取消
                  </button>
                </div>
...
function Stat({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-[28px] border border-pink-100 bg-white p-5">
      <p className="text-sm text-[#80647d]">{title}</p>
...
function MiniStat({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-2xl border border-pink-100 bg-white p-4">
      <p className="text-xs text-[#80647d]">{title}</p>
      <p className="mt-2 text-lg font-black text-[#7b4f85]">{value}</p>
    </div>
  );
}
...
function Input({
  ...
  return (
    <label className="block">
      <span className="text-sm text-[#6f526d]">{label}</span>
      ...
      <input
        ...
        className="mt-2 w-full rounded-xl border border-pink-100 bg-pink-50 px-4 py-3 text-[#3f2947] outline-none placeholder:text-zinc-600 focus:border-pink-400"
      />
    </label>
  );
}
...
function SearchableStaffSelect({
  ...
  return (
    <div className="block">
      ...
      <div className="mt-2 flex items-center gap-2 rounded-xl border border-pink-100 bg-pink-50 px-4 py-3 focus-within:border-pink-400">
      ...
      {selectedStaff ? (
        <div className="mt-2 flex items-center justify-between gap-3 rounded-xl border border-pink-200 bg-pink-50 px-4 py-3">
          <div>
            <p className="text-sm font-bold text-[#7b4f85]">
              {getDisplayStaffName(selectedStaff)}
            </p>
            <p className="text-xs text-[#92778f]">{selectedStaff.discord_id}</p>
          </div>
