...
export default function StaffPage() {
  ...
  async function transferSalaryToAsd() {
    ...
    if (amount > available) {
      alert(`轉入金額不能超過可提領薪資 $${available.toLocaleString()}`);
      return;
    }

    if (
      !confirm(
        `確定要將 $${amount.toLocaleString()} 薪資轉入你本人的 ASD？\n轉入後會立即扣除可提領薪資並增加 ASD 餘額。`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setWithdrawAmount("");
      alert(
        `已轉入本人 ASD；目前 ASD 餘額 $${Number(
          payload.asdBalance || 0
        ).toLocaleString()}`
      );
    } catch (error: unknown) {
      console.error("transfer salary to ASD error:", error);
      alert(error instanceof Error ? error.message : "轉入 ASD 失敗");
    } finally {
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <div className="mt-2 space-y-1 text-sm leading-6 text-[#8b5a8f]">
                  ...
                  <p>客服底薪與客服接待件數薪資統一於次月 2 日 00:00 入帳。</p>
              ...
              <div className="flex w-full flex-col items-stretch gap-2 sm:max-w-xs sm:items-end">
                ...
                <div className="grid w-full grid-cols-2 gap-2">
                  <button
                    ...
                    className="qiunai-button w-full px-4 py-3 font-bold disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {withdrawing ? "申請中..." : "提領"}
                  </button>
                  <button
            ...
            {walletLoading && !salaryWallet ? (
              ...
              <>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#8b5a8f]">
                      ...
                      {salaryWallet.totals.pendingWithdrawn > 0
                        ? `，申請中：$${Number(
                            salaryWallet.totals.pendingWithdrawn || 0
                          ).toLocaleString()}（${salaryWallet.totals.pendingRequestCount} 筆）`
                        : ""}
                    </p>
                  </div>
                ...
                <div className="mt-4 rounded-[24px] border border-pink-100 bg-white/90 p-4">
                  <h3 className="font-black text-[#5b3768]">每月錢包總額</h3>
                  <p className="mt-1 text-xs text-[#8b5a8f]">
                    ...
                    <table className="min-w-full text-sm">
                      ...
                      <tbody>
                        {salaryWallet.monthlySummaries.map((summary) => (
                          <tr key={summary.month} className="border-t border-pink-100">
                            <td className="py-3 pr-4 font-bold text-[#5b3768]">
                              {summary.month.replace("-", " 年 ")} 月
                            </td>
                            <td className="py-3 pr-4 font-black text-emerald-600">
                              +{statementMoney(summary.deposited)}
                            </td>
                            <td className="py-3 pr-4 font-black text-[#5b3768]">
                              -{statementMoney(summary.withdrawn)}
                            </td>
                            <td className="py-3 font-bold text-amber-600">
                              {statementMoney(summary.pendingWithdrawn)}
                            </td>
                          </tr>
                        ))}
                        {salaryWallet.monthlySummaries.length === 0 ? (
                          <tr><td colSpan={4} className="py-4 text-[#8b5a8f]">目前沒有錢包紀錄</td></tr>
                        ) : null}
                      </tbody>
                    </table>
                  </div>
                  ...
                  {statementData ? (
                    <div className="mt-4">
                      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <WalletStat
                          title="扣除費用"
                          value={statementMoney(statementData.summary.serviceFee)}
                        />
                        <WalletStat
                          title="已完成實際入帳"
